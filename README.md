...
Современное REST API для блог-платформы, написанное на Go с использованием чистой архитектуры и современных технологий. Включает систему rate limiting на базе Redis для защиты от атак и спама.

## 🚀 Описание проекта

Blog API - это полнофункциональное веб-приложение для создания и управления блог-постами с системой аутентификации, комментариями и пользовательскими профилями. Проект построен с использованием принципов чистой архитектуры и следует лучшим практикам разработки на Go. Включает в себя систему rate limiting для защиты от брутфорс атак и спама с использованием Redis.
...
- **Rate Limiting** (`internal/service/`) - сервисы для ограничения попыток логина и комментариев с использованием Redis
...
- **Redis-based**: все ограничения хранятся в Redis с автоматическим истечением
- **Изолированные лимиты**: блокировка комментариев к одному посту не влияет на другие посты
- **Автоматическая разблокировка**: по истечении времени окна пользователь автоматически разблокируется
...
### Добавление rate limiting для новых endpoints:

1. Создайте Redis limiter в `internal/repository/`
2. Создайте сервис в `internal/service/`
3. Интегрируйте в API handler
4. Настройте лимиты в конфигурации сервиса
...
- **Redis rate limiting**: быстрая проверка ограничений без обращения к БД
- **In-memory кеширование**: мгновенный доступ к данным блокировки
- **Автоматическое управление памятью**: Redis автоматически очищает истекшие ключи
...
- **Rate Limiting** для защиты от брутфорс атак и спама
  - **Защита логина**: блокировка после 3 неудачных попыток входа
  - **Защита от спама**: ограничение комментариев к одному посту
  - **Redis-хранилище**: быстрый доступ к данным блокировки
  - **Автоматическое истечение**: блокировки снимаются автоматически
